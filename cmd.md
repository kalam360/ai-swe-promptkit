...
- **Plan:**
   ...
   3.  Update files in the `task-tracking` directory. These files are used by the `ai-swe status` command (via `prompts/gen_status.md`) to generate the `blueprint/impl_status.md` file.
- **Execution:**
...
- **Execution:**
    1. Update the status of the task to "In Progress" in `blueprint/task-tracking/tasks.md` and/or `blueprint/task-tracking/current_sprint.md`. This update will be reflected in the `blueprint/impl_status.md` file generated by the `ai-swe status` command.
...
When you use the `ai-swe status` command, the AI, acting as a project auditor, will generate a comprehensive status report, including an implementation status overview in `blueprint/impl_status.md`.
...
- Additionally, uses the above files to generate:
  * blueprint/impl_status.md (implementation status with checkboxes)

**AI Agent Behavior:**

- **Role:** Project auditor.
- **Plan:**
    1. Understand all blueprint documents. Compare the current state of the project with the planned state. Identify any discrepancies, risks, or issues.
    2. Use `prompts/gen_status.md` to generate `blueprint/impl_status.md`, tracking the status of features, modules, and tasks with checkboxes.
- **Execution:**
    1. Generate a general status report summarizing the project's progress, including completed tasks and user stories, tasks in progress, upcoming tasks, and any identified risks or issues.
    2. Create or update `blueprint/impl_status.md` with detailed status information using checkboxes, as defined in `prompts/gen_status.md`.
...
5. Status Reporting Flow:
   gen_status.md -> blueprint/tasks.md, blueprint/user_stories.md, blueprint/modules.md, blueprint/task-tracking/* -> blueprint/impl_status.md